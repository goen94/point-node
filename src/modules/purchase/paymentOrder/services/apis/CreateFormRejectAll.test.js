describe('Payment Order - CreateFormRejectAll', () => {
  describe('reject fail', () => {
    it('throw error if reason is empty', async () => {
      ...
      await expect(async () => {
        await new CreateFormRejectAll(tenantDatabase, token, createFormRejectDto).call();
    ...
    it('throw error when rejected by unwanted user', async () => {
      ...
      const token = await createToken(paymentOrder, hacker);
  ...
  describe('reject success', () => {
    it('update form status to rejected', async () => {
      const { paymentOrder, approver, paymentOrderForm } = await generateRecordFactories();
      const createFormRejectDto = {
        reason: faker.datatype.string(200),
      };
    ...
    describe('check form reference pending', () => {
      ...
      beforeEach(async (done) => {
        ...
        ({ ids } = await new CreateFormRejectAll(tenantDatabase, token, createFormRejectDto).call());
