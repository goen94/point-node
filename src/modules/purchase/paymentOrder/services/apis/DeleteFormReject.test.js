describe('Payment Order - DeleteFormApprove', () => {
  ...
  describe('reject success', () => {
    it('update form status to rejected', async () => {
      ...
      ({ paymentOrderForm } = await new DeleteFormReject(tenantDatabase, {
        approver,
        paymentOrderId: paymentOrder.id,
        deleteFormRejectDto,
      }).call());

      expect(paymentOrderForm.cancellationStatus).toEqual(-1);
    });

    describe('check form reference pending', () => {
      beforeEach(async (done) => {
        const { approver, paymentOrder } = await generateRecordFactories();
        ({ paymentOrder } = await new DeleteFormReject(tenantDatabase, { approver, paymentOrderId: paymentOrder.id }).call());
