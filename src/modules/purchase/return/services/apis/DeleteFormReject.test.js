...
describe('Purchase Return - DeleteFormReject', () => {
  ...
  it('success reject form', async (done) => {
    ...
    const { approver, chartOfAccountPayable, chartOfTax } = recordFactories;
    ...
    request(app)
      .post('/v1/purchase/return/' + purchaseReturn.id + '/cancellation-reject')
      ...
      .expect(async (res) => {
        ...
        const journalTax = await tenantDatabase.Journal.findOne({
          formId: purchaseReturnForm.id,
          chartOfAccountId: chartOfTax.id,
        });
